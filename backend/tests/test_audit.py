...
class TestAuditService:
    ...
    def test_get_logs_filter_by_action(self) -> None:
        self.svc.log(action="create", resource_type="system")
    ...
    def test_get_logs_filter_by_resource_type(self) -> None:
        self.svc.log(action="create", resource_type="system")
        self.svc.log(action="create", resource_type="incident")
        logs = self.svc.get_logs(resource_type="incident")
        assert len(logs) == 1

    def test_get_logs_filter_by_user_id(self) -> None:
    ...
    def test_log_failure_status(self) -> None:
        entry = self.svc.log(
            action="login",
            resource_type="auth",
            status="failure",
...
class TestAuditAPI:
    ...
    def test_get_audit_logs_empty(self, _mock_logs: AsyncMock, client: TestClient) -> None:
        resp = client.get("/api/audit/logs")
        assert resp.status_code == 200
        assert isinstance(resp.json(), list)

    @patch("apps.crud.get_audit_logs", new_callable=AsyncMock, return_value=[])
    def test_export_audit_logs(self, _mock_logs: AsyncMock, client: TestClient) -> None:
    ...
    def test_get_incident_logs(self, _mock_logs: AsyncMock, client: TestClient) -> None:
